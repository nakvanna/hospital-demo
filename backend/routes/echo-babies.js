...
/*router.get('/get', async (req, res, next) => {
  try {
    await Echo.find().populate('patient').exec(function (err, echo) {
      res.json({
        status: true,
        data: echo,
        message: 'Successfully fetched!'
...
//   try {
//     await EchoBaby.findById(req.params.id).select('_id').populate('patient', 'lname fname').exec(function (err, echo_babies) {
//       res.json({
//         status: true,
//         data: echo_babies,
//         message: 'Successfully fetched!'
